...
class Staffs extends Controller
{
    public function index()
    {
        ...
        // 3️⃣ Map cityName và wardName cho mỗi staff
        // tạo map trước
        $cityMap = [];
        foreach ($cities as $c) {
            $cityMap[$c['id']] = $c['name'];
    ...
    public function add_staff()
    {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT id FROM roles WHERE name = 'STAFF' LIMIT 1");
            $stmt->execute();
            $result = $stmt->get_result();
    ...
    public function search_staff()
    {
        ...
        $staffs = $staffsModel->join_multi(
            joins: [
                ['table' => 'users', 'type' => 'LEFT', 'on' => 'staffs.id = users.id']
            ],
            select: '
            staffs.*,
            users.fullName,
        ...
        if (!empty($search) || !empty($status)) {
            $staffs = array_filter($staffs, function ($staff) use ($search, $status) {
        ...
        echo json_encode(['success' => true, 'staffs' => array_values($staffs)]);
        exit;
    }
    ...
    public function update_staff()
    {
        ...
        try {
            ...
            $staffsModel = new class extends myModels {
            ...
            if (!empty($staffData)) {
                $existing = $staffsModel->findOne(['id' => $input['id']]);
                if ($existing) {
                    $staffResult = json_decode(
                        $staffsModel->update($staffData, ['id' => $input['id']]),
                        true
                    );
                } else {
                    $staffData['id'] = $input['id'];
                    $staffResult = json_decode(
                        $staffsModel->insert('staffs', $staffData),
                        true
                    );
                }
    ...
    public function toggleStatus_staff()
    {
